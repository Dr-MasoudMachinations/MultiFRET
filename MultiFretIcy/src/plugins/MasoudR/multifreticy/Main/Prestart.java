 ...
 *   images/Save16.gif
 */
public class Prestart implements ActionListener, AcquisitionListener, ItemListener, ChangeListener {
	...
	/* GUI for selection and loading tools for application preferences */
...
//        checkboxPanel.add(mpCheckBox); //Deprecated, always true
		...
		openButtonChannel = new JButton("Select Contour-file", createImageIcon("images/Open16.gif"));
		openButtonChannel.addActionListener(this);

		calcsButton = new JButton("Select Calculations", createImageIcon("images/Open16.gif"));
		calcsButton.addActionListener(this);

		// Same for the other file opening button
		openButtonTransfo = new JButton("Open a File...", createImageIcon("images/Open17.gif"));
		openButtonTransfo.addActionListener(this);

		// Add Listener for tab limit spinner
		tabSpinner = new JSpinner(new SpinnerNumberModel(25, 1, 1000, 1));
		tabSpinner.addChangeListener(this);

		// Create the save button.
		saveButton = new JButton("Save Settings...", createImageIcon("images/Save16.gif"));
		saveButton.addActionListener(this);

		// Textbox for excel output URL
		outputLocation = new JTextField(System.getProperty("user.home") + "\\Datasheet.xlsx");

		browseButton = new JButton("Open a File...", createImageIcon("images/Browse16.gif"));
		...
		outputPanel.add(new JLabel("Tab limit:"));
		outputPanel.add(tabSpinner);
...
//        transfoEnable.setVisible(false);
		...
		try {
			...
			outputLocation
					.setText(cp.getProperty("outputLocation", System.getProperty("user.home") + "\\Datasheet.xlsx"));

			// Open tranfofile
			if (tfURL != null) {
				transfoFile = new File(tfURL);
				if (transfoFile == null) {
					log.append("unable to open transformation file" + newline);
				} else {
			...
			if (cfURL != null) {
				...
				if (channelFile == null) {
					log.append("unable to open contour file" + newline);
				} else {
			...
			if (mfURL != null) {
				...
				if (calcsFile == null) {
					log.append("unable to open calcs file" + newline);
				} else {
...
//			mpBool = Boolean.parseBoolean(cp.getProperty("multipos", "true")); //Deprecated, always true
			...
			tabLimit = Integer.parseInt(cp.getProperty("tablimit", "25"));
...
//			mpCheckBox.setSelected(mpBool); //Deprecated, always true
		...
		mainFrame.setOkAction(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) // On OK click
			{
				...
				for (Sequence sequence : sequences) {
					...
					if (loadRois(channelFile, sequence)) {
						log.append("Loaded ROIs from " + channelFile.getName() + newline);
					} else {
	...
	public void actionPerformed(ActionEvent e) {
		...
		// Handle open button action.
			...
			cp.setProperty("tablimit", String.valueOf(tabLimit));

			cp.setProperty("outputLocation", outputLocation.getText());
			if (transfoFile != null) {
				cp.setProperty("transfofile", transfoFile.getPath());
			}
			// else {log.append("No transformation file selected." + newline);}
			if (channelFile != null) {
				cp.setProperty("contourfile", channelFile.getPath());
			}
			// else {log.append("No contour file selected." + newline);}
			if (calcsFile != null) {
				cp.setProperty("calcsfile", calcsFile.getPath());
			}
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				if (file.isDirectory()) {
					outputLocation.setText(file.getAbsolutePath() + "\\" + "Datasheet.xlsx");
				} else {
...
//    }
	...
	public void stateChanged(ChangeEvent e) {
		// Tab Limit spinner
		if (e.getSource() == tabSpinner) {
			tabLimit = (int) tabSpinner.getValue();
			log.append("Tab Limit set to " + String.valueOf(tabLimit) + newline);
		}
	}
}
