...
/* Splits image into cropped images based on number of channel ROI */
...
public class Splitter {
	...
	Splitter(ArrayList<Sequence> acquiredSeqs, ArrayList<ArrayList<MyCoordinates>> allCorners)
		...
		for (Sequence acquiredSeq : acquiredSeqs) {
			...
			for (int r = 0; r < rois.size(); r++) {
				...
				IcyBufferedImage img = new IcyBufferedImage((int) Math.round(bnd.getSizeX()),
						(int) Math.round(bnd.getSizeY()), 1, dataType);
			...
			if (allCorners != null && MultiFretIcy.PS.transformEnabled == true) {
				for (Sequence seq : seqBundle) {
					/*
					 * we get roi name from xml file in cornerfinder here we need to get image + roi
					 * name to send to transformer along with corners
					 */
