...
/* Holds methods to convert images to openCV compatible formats and back, and perform affine transformation. */

class AffineWarp {
	AffineWarp(ArrayList<MyCoordinates> mc, Sequence seq) {
	...
	BufferedImage GeometricTransforms(ArrayList<MyCoordinates> mc, Sequence seq) {
	...
	public double getAngle(Point[] pointArray) {
		double pointDistance = Math
				.sqrt(Math.pow(pointArray[1].x - pointArray[0].x, 2) + Math.pow(pointArray[1].y + pointArray[0].y, 2));
		int k = 1;
		for (int i = 2; i < pointArray.length; i++) {
			double pointDistance2 = Math.sqrt(
					Math.pow(pointArray[i].x - pointArray[0].x, 2) + Math.pow(pointArray[i].y + pointArray[0].y, 2));
