...
public class CornerFinder extends JPanel {
	...
	public CornerFinder() {
		/*
		 * 1. Convert ROI into arraylists 2. For each point, we go forward 15 points and
		 * backwards 15 points 3. We check that all these points are along one line
		 * (hurrahs) 4. The line is added to a list, and this process is repeated until
		 * all lines are found ** multiple lines may be found alone the same edge of a
		 * polygon, this results in an error (probably means the edge was too distorted)
		 * -> TODO: could make it so similar lines are averaged 5. We use the intersect
		 * of these lines to create a corner ** if 4 lines are not found, we get an
		 * error
		 */
	}
	...
	public ArrayList<ArrayList<MyCoordinates>> Aktivat(File contourFile) {
		...
		try {
			...
			for (int roiNum = 0; roiNum < rList.getLength(); roiNum++) {
				// Find ROI name in XML file and obtain element containing points
				...
				for (int temp = 0; temp < nList.getLength(); temp++) {
					...
					if (nNode.getNodeType() == Node.ELEMENT_NODE) {
						...
						MyCoordinates xy = new MyCoordinates(
								Double.parseDouble(eElement.getElementsByTagName("pos_x").item(0).getTextContent()),
								Double.parseDouble(eElement.getElementsByTagName("pos_y").item(0).getTextContent()),
								roiName);
						if (lowX == 0 || xy.getX() < lowX) {
							lowX = xy.getX();
						}
						if (highX == 0 || xy.getX() > highX) {
							highX = xy.getX();
						}
						if (lowY == 0 || xy.getY() < lowY) {
							lowY = xy.getY();
						}
						if (highY == 0 || xy.getY() > highY) {
							highY = xy.getY();
						}
						xmlPoints.add(xy);
					}
				...
				/*
				 * For loop iterating each Point. For a select point, we set a current, prior
				 * and posterior point. We calculate the angle between these points.
				 */
				for (int temp = 0; temp < xmlPoints.size(); temp++) {
					...
					System.out.println("\nCurrent Element-1 : " + t);
					if (xmlPoints.get(t).stopper) {
						System.out.println("stopper found");
						continue;
					}
					...
					// a posteriori
					t = temp;
					if (t + spreadvar > xmlPoints.size() - 1) {
						t = (t + spreadvar) - (xmlPoints.size());
					} else {
						t = temp + spreadvar;
					}
					System.out.println("\nCurrent Element+1 : " + t);
					...
					System.out.println(angle);
					if (angle < 91) {
						System.out.println("Possible corner");
					}
					System.out.println("----------------------------");
					System.out.println("----------------------------");
					if (count1 == 1) {
						...
						// While between the two lines
						while (hurrahTrain == 1) {
							if (lastT - 1 < 0) {
								lastT = xmlPoints.size() + lastT - 1;
							} else {
								lastT = lastT - 1;
							}
							...
							if (lastT == firstT) {
								...
								System.out
										.println("x2,y2= " + firstX + "," + firstY + " x3,y3= " + lastX + "," + lastY);
							...
							if (xmlPoints.get(lastT).stopper) {
								System.out.println("stopper found, switching direction");
							}
							if (d < distanceLimit && !xmlPoints.get(lastT).stopper) {
								...
								// While between the two lines
								while (hurrahTrain == 1) {
									if (lastT + 1 > xmlPoints.size() - 1) {
										lastT = (lastT + 1) - (xmlPoints.size());
									} else {
										lastT = lastT + 1;
									}
									...
									if (lastT == firstT) {
										...
										System.out.println("hurrah " + hurrah);
										System.out.println(
												"x2,y2= " + firstX + "," + firstY + " x3,y3= " + lastX + "," + lastY);
									...
									if (xmlPoints.get(lastT).stopper) {
										System.out.println("stopper found, finalising");
									}
									if (d2 < distanceLimit && !xmlPoints.get(lastT).stopper) {
										...
										if (ac.size() > hurrahLimit) {
											...
											/*
											 * get slope a = y1-y2 / x1-x2 y = ax+b b = y-ax intersect: ax+b = cx+d,
											 * solve x = the intersect x input in y=ax+b for y how to code: intersect =
											 * x = (d-b)/(a-c)
											 * 
											 */
											if (!xmlPoints.get(lastT).stopper) {
											...
											System.out.println(
													"Codordonets:" + firstX + "," + firstY + " " + lastX + "," + lastY);
	...
	public ArrayList<MyCoordinates> FindCorners(ArrayList<MyCoordinates> pList, double lowX, double highX, double lowY,
			double highY) {
		ArrayList<MyCoordinates> corners = new ArrayList<MyCoordinates>();
		...
		for (int i = 0; i < pList.size(); i++) { // For each slope/intercept in the list
			for (int j = i + 1; j < pList.size(); j++) { // Compare to each other pair
				...
				for (int k = 0; k < pList.get(i).getPoints().size(); k++) {
					// See if these pairs have a common point
					for (int l = 0; l < pList.get(j).getPoints().size(); l++) {
						// If so, abort.
						if (pList.get(j).getPoints().get(l).getX() == pList.get(i).getPoints().get(k).getX()
								&& pList.get(j).getPoints().get(l).getY() == pList.get(i).getPoints().get(k).getY()) {
							goAhead = 0;
							System.out.println("Common point found");
						}
						if (pList.get(i).getX() * pList.get(j).getPoints().get(l).getX() + pList.get(i).getY() == pList
								.get(j).getPoints().get(l).getY()) {
				...
				if (goAhead == 1) {
					double intersectX = (pList.get(j).getY() - pList.get(i).getY())
							/ (pList.get(i).getX() - pList.get(j).getX());
		...
		for (int m = 0; m < corners.size(); m++) {
			System.out.println(corners.get(m).getX() + " " + corners.get(m).getY());
		}
		return corners;
	}
	...
	public Path2D PathPolygon(ArrayList<MyCoordinates> points) {
		...
		path.closePath();
		System.out.println("Bounds: " + path.getBounds().getX() + " " + path.getBounds2D().getY() + " "
