...
/* Cell detector / dish scanner work in progress */
...
public class Detector {
	...
	public Detector(String name) throws Exception {
		...
		MessageDialog.showDialog("The automated cell detection feature is currently in development, "
				+ "this feature is incomplete and its use is not recommended.");
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < picList.getModel().getSize(); i++) {
					...
					if (pp.getSel()) {
						...
						MultiStagePosition ppPos = new MultiStagePosition(StageMover.getXYStageDevice(),
								pp.getPos().getX(), pp.getPos().getY(), StageMover.getZFocusDevice(), pp.getZFocus());
...
//    	scrollPane.createVerticalScrollBar();
	...
	private ArrayList<PicturePosition> scanField() throws Exception {
		...
		if (tlPos == null || brPos == null || tlPos == brPos) {
			new ToolTipFrame("Set bottom-left and top-right of your dish first!", 10);
			return null;
		} else {
			System.out.println("TL: " + tlPos + " BR: " + brPos);
		}
		...
		(alty > 0 && StageMover.getXY().getY() >= tlPos.getY() && StageMover.getXY().getY() <= brPos.getY())
				|| (alty < 0 && StageMover.getXY().getY() <= tlPos.getY()
						&& StageMover.getXY().getY() >= brPos.getY())) {
			if (abort) {
			...
			(altx > 0 && StageMover.getXY().getY() >= tlPos.getY() && StageMover.getXY().getY() <= brPos.getY())
					|| (altx < 0 && StageMover.getXY().getY() <= tlPos.getY()
							&& StageMover.getXY().getY() >= brPos.getY())) {
				if (abort) {
					abort = false;
					SwingUtilities.invokeLater(new Runnable() {
				...
				if (!focusBox.isEnabled()) {
					list.add(new PicturePosition(MicroManager.snapImage(), StageMover.getXY()));
				} else {
					list.add(new PicturePosition(MicroManager.snapImage(), StageMover.getXY(), StageMover.getZ()));
				}
				StageMover.moveXYRelative(
						altx * Double.parseDouble(viewportW.getText()) * Double.parseDouble(pixelSize.getText()), 0);
			}
			System.out.println("New row");
			StageMover.moveXYAbsolute(currentTL.getX(), currentTL.getY());
			StageMover.moveXYRelative(0,
					alty * Double.parseDouble(viewportH.getText()) * Double.parseDouble(pixelSize.getText()));
	...
	private void pixelSizer() throws Exception {
		...
		case 2:
			...
			new ToolTipFrame(
					"Enter viewport width and height, then click Finish to obtain pixel-Size in Micro-Manager's used units.");
			findPStatus = 3;
			break;
		case 3:
			findPStatus = 0;
			...
			pixelSize.setText(Double.toString((distX / Double.parseDouble(viewportW.getText()))
	...
	public void addToPane(JList<PicturePosition> pL) throws Exception {
		...
		for (int i = 0; i < pL.getModel().getSize(); i++) {
			...
			Border b1 = new BevelBorder(BevelBorder.LOWERED, Color.GREEN, Color.BLUE);
